...
| D8 | **No duplicate names.** A season's name is unique; a player is unique by name and alias together — two players may share a name if their aliases differ. Enforced by validation and a unique index. | Product owner, 2026-09-24. |
...
| D11 | **How the statistics count** where STAT-* leaves a detail open: the form's thirds round up (of 13 places, 1–5 good, 9–13 bad); the average place of the all-time balance takes only seasons with at least one complete matchday; a best or worst matchday reached twice lists both; tied record holders appear in the order of the seasons (newest first), the matchdays, then the overall table. | Chosen while implementing, 2026-09-24; not yet confirmed by the product owner. |
