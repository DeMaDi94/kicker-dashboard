...
| D14 | **How visits are counted** (VIS-01–06). A visit is a successful (200) `GET` of a public page by a guest, counted after the response; an Inertia prefetch or partial reload is not a visit. A bot is a request with no user agent or one matching `bot`, `crawl`, `spider`, `slurp`, `facebookexternalhit`, `preview`, `monitor`, `headless`, `curl`, `wget`, `python` or `go-http-client` (any case). The visitor mark is SHA-256 of a random salt of the day with the IP and the user agent, cut to 16 hex digits; the salt lives in the cache until midnight and is never stored with the visits, so a mark cannot be traced back once the day is over. Old visits are pruned nightly (`model:prune`). Admins see the screen through the permission `visits.view` at `/visits`, the last entry of the navigation; the chosen period is the `days` query, and any other value opens the 30 days. The pages list all four public pages, one nobody called with 0; equal counts keep the order of VIS-01. | Product owner, 2026-09-25 (answers to the visit-statistics questions; the navigation place, the page list and the fallback period the same day); the bot list and the hash are the implementation's choice under „no bots“ and „also unique visitors“. |
| D15 | **An admin creating a user chooses how the password is set:** send the invitation (B14, pre-selected) or set the password in the form, entered twice and validated as any password is. A password set by the admin sends no mail and marks the address verified, as `users:set-password` does (D13); the user is not made to change it at the first sign-in. | Product owner, 2026-09-25. Supplements B14 and replaces D13's „the invitation stays the way accounts get their password“. |
| D16 | **Outgoing mail can be switched off.** One switch in a settings section „E-Mail“, stored in the database and seen only with the permission `settings.manage` (admins); a new installation starts with it off. While it is off nothing is mailed, and every screen that would send one says so rather than failing: „Forgot password“ is hidden from the login page and its routes answer 404, the invitation cannot be chosen when creating a user (setting the password, D15, is pre-selected), the admin's reset-link button (B16) and the resend of a verification mail are disabled, and a user cannot change their own address — no mail could confirm it, and an unverified address would lock them out. `users:create-admin` then asks for a password (`--password`) and sets it as D15 does. | Product owner, 2026-09-25 („as it's not set up at the moment I want to disable all outgoing email features“). |
| D17 | **How player editing, news and the history work** (PLY-02, NEWS-01–03, LOG-01–03). Editing a player needs the permission `players.update`, changing or deleting another's news `news.manage`; both belong to admins. News: at most 2 000 characters; deleting one asks first, as deleting a season does; an edited post shows „bearbeitet“ beside its date; the author's name stays when their account is deleted (B15); a deleted season's news goes and comes back with it (SEA-06). History: one save is one entry — a matchday save lists every changed point in it; 20 entries per page, as the user list (B16); admins see it through the permission `history.view` at `/history`, whose navigation entry „Verlauf“ now comes last, after „Besucher“ (LOG-03; D14 had „Besucher“ last); a change nobody signed in made (a console command) names no user; it starts empty — what happened before it existed is not reconstructed. | Product owner, 2026-09-25 (confirmed the proposed details with „implement“). |
| D18 | **How a matchday is shared** (MD-06, MD-07). The button „Teilen“ sits in the matchday's panel of the season view and only shows for a complete matchday. The browser builds the text from the season view's props and hands it to the device's share menu (Web Share API), the link inside the text; without that menu, or when it fails, the text goes to the clipboard and a toast says „Kopiert“ — closing the menu is not a failure. Names are the short names without the alias (D4 is for the screen); a tied place repeats its number (MD-03); the product name is `config('app.name')` (D1). The link is `/seasons/{id}?matchday={n}`, not `/`, which moves on to the newest season (D6). The preview is Open Graph tags the server writes into the page's HTML (`og:title`, `og:description`, `og:image`, `og:url`), only when the address names a complete matchday; the image is `public/og-image.png`, the mark (D9) on the red ground, 1200 × 630. A messenger fetching the preview is a bot (VIS-01): D14's word list gains `whatsapp`; Telegram, Discord, Slack and iMessage already match `bot` or `facebookexternalhit`. | Product owner, 2026-09-25 (the whole matchday result, everyone may share, a fixed image, previews not counted). |
