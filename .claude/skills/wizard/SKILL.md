...
license: MIT
metadata:
  source: mattpocock/skills@1.2.2 skills/engineering/wizard
  adapted: "Scoping reads config/ and CI instead of .env files, whose reads are denied in this repo; the human runs the script."
---
...
- For setup: `config/*.php` (every `env('…')` call is a value the wizard may need to produce), `README`, `docker-compose*`, and `.github/workflows/*` (every `secrets.*` / `vars.*` reference is a value the wizard must produce). Reads of `.env` and `.env.*` are denied in this repo — don't try; ask the user which keys already have values. The script writes `.env` when the human runs it, never you.
