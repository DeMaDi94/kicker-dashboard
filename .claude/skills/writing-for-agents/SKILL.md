...
license: MIT
metadata:
  source: mattpocock/skills@1.2.2 skills/productivity/writing-for-agents
  adapted: "Adds a note that this repo uses CLAUDE.md plus path-scoped .claude/rules/ instead of AGENTS.md."
---
...
This repo has no `AGENTS.md`: the always-loaded document is `CLAUDE.md`, and path-scoped reference lives in `.claude/rules/*.md`, loaded when a matching file is touched (the map is in `.claude/README.md`). Read `AGENTS.md` below as `CLAUDE.md`, and a rule file as disclosed reference whose pointer is its `paths:` glob.
